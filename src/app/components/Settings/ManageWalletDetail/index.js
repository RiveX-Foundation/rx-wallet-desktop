...
@inject((stores) => ({
  ...
  setSelectedPrivateAddress: (privateaddress) =>
    stores.setting.setSelectedPrivateAddress(privateaddress),
  selectedwalletaddress: stores.setting.selectedwalletaddress,
  removeWallet: (publicaddress) =>
    stores.walletStore.removeWallet(publicaddress),
  changeWalletName: (publicaddress, newwalletname) =>
    stores.walletStore.changeWalletName(publicaddress, newwalletname),
  ...
  removeBackendWallet: (password) =>
    stores.walletStore.removeBackendWallet(password),
}))
@observer
class ManageWalletDetail extends Component {
  ...
  componentDidMount() {
    var wallet = this.props.walletlist.find(
      (x) => x.publicaddress == this.props.selectedwalletaddress
    );
    this.setState({
  ...
  getRate = async () => {
    ...
    await this.props.getConvertRate(
      this.props.selectedTokenAsset.AssetCode,
        ...
        (err, res) => {
          if (res) {
            ...
            if (authcode) {
              var wallets = JSON.parse(localStorage.getItem("wallets"));
              if (
                wallets[0].publicaddress == this.state.selectedwalletaddress
              ) {
                createNotification(
                  "success",
                  "Removed dApp wallet! The app will relaunch in 5 seconds"
                );
                this.setState({
                  removemodalvisible: false,
                });
                ...
                createNotification(
                  "success",
                  "Successfully removed the wallet!"
                );
                this.props.removeWallet(this.state.selectedwalletaddress);
                this.setState({ password: "" });
                this.back();
  ...
  exportprivatekey = (e) => {
    ...
    this.setState({
      ...
      exportprivatekeymodalvisible: true,
    });
  };

  exportmnemonic = (e) => {
    const wallet = this.props.walletlist.find(
      (x) => x.publicaddress == this.props.selectedwalletaddress
    );
    if (localStorage.getItem("twofasecret") != null) {
      this.setState({
    ...
    this.setState({
      selectedwalletaddress: walletaddress,
      exportmnemonicmodalvisible: true,
    });
  };
  ...
  handleChangeWalletNameOk = () => {
    this.setState(
      ...
      () => {
        this.props.changeWalletName(
          this.props.selectedwalletaddress,
  ...
  selectToken = (e) => {
    this.setState({
      ...
      temporarypublicaddress: e.currentTarget.getAttribute(
        "data-publicaddress"
      ),
    });
    console.log(
      "private address: " +
        e.currentTarget.getAttribute("data-privateaddress").toString()
    );
    this.props.setSelectedPrivateAddress(
      e.currentTarget.getAttribute("data-privateaddress")
    );
  };
          ...
          <div className="">
            <div className="title">
              ...
              <span style={{ marginLeft: "10px" }}>
                {intl.get("Settings.ManageWallets")}
              </span>
            </div>
            ...
            {wallet.holders != null && wallet.wallettype == "sharedwallet" && (
              <div className="spacebetween" style={{ width: "600px" }}>
                <div
                  className="panelwrapper borderradiusfull signerspanel"
                  style={{ marginBottom: "10px", padding: "10px" }}
                >
                  <div className="panellabel coplayertitle">
                    {intl.get("Wallet.CoPlayers")} {wallet.totalsignatures}-
                    {wallet.totalowners}
                  </div>
                  ...
                  {wallet.wallettype == "sharedwallet" &&
                    wallet.holders.map(function (item, i) {
                      return (
                        <div key={i} className="panellabel logname">
                          <span className="multisigtick">
                            <img src={buttonmultisigtick} />
                          </span>
                          {item.UserName}
                        </div>
                      );
              ...
              <React.Fragment>
                ...
                >
                  <div className="panellabel">
                    {intl.get("Settings.ExportPrivateKey")}
                  </div>
                  <div className="panelvalue">
                    <img
                      style={{ cursor: "pointer" }}
                ...
                <div
                  onClick={this.exportmnemonic}
                  className="panelwrapper borderradiusfull spacebetween"
                  style={{ marginBottom: "30px" }}
                >
                  <div className="panellabel">
                    {intl.get("Settings.ExportMnemonic")}
                  </div>
                  <div className="panelvalue">
                    <img
                      style={{ cursor: "pointer" }}
            ...
            <div
              onClick={this.removewallet}
              data-walletname={wallet.walletname}
            ...
            >
              <p className="modalcontent">
                {intl
                  .get("Modal.AreYouSureRemoveWallet")
                  .replace("{walletname}", this.state.selectedwalletname)}
              </p>
            ...
            >
              ...
              <div>
                <div className="pmodalcontent">
                  {intl.get("Settings.ExportPrivateKey.Msg1")}
                </div>
                <div className="pmodalcontent">
                  {intl.get("Settings.ExportPrivateKey.Msg2")}
                </div>
                <div className="pmodalcontent">
                  {intl.get("Settings.ExportPrivateKey.Msg3")}
                </div>

                <div className="pmodalcontent">
                  {intl.get("Settings.ExportPrivateKey.SelectTokenAsset")}
                </div>
                <center>
                  <div
                  ...
                  >
                    <div className="panelwrapper">
                      <Input
                        value={this.state.temporarytokenname}
                        placeholder={intl.get("Settings.Token")}
            ...
            >
              ...
              <div>
                <div className="pmodalcontent">
                  {intl.get("Settings.ExportMnemonic.Msg1")}
                </div>
                <div className="inputpanel">
                  <center>
                    <div className="panelwrapper borderradiusfull">
                ...
                <div className="pmodalmnemonickey" style={mnemonicvisibility}>
                  {this.state.exportedseedphrase}
                </div>
              </div>
            </Modal>
            ...
            >
              ...
              <div className="pmodalcontent">
                {" "}
                {intl.get("Settings.Newwalletname")}
                <div className="panelwrapper borderradiusfull">
                  <Input
                    className="inputTransparent"
