...
class Language extends Component {
    ...
    render() {
        ...
        if (originallist.length == 0) {
            ...
            name = originallist.find(x => x.shortcode == language).name;
        }
        return (
                ...
                <div className="centerpanel">
                    <div className="inputwrapper">
                        <div className="panelwrapper">
                            <Input ref={(input) => {
                                this.inputEl1 = input;
                            }} value={name} id="language" placeholder={intl.get('Settings.Language')}
                                   onClick={this.showlist} onBlur={this.hidelist} onFocus={this.showlist}
                                   className="inputTransparent" onChange={this.setfilterlist}/>
                            <div className={autoliststyle}>
                                {filterlist.length > 0 &&
                                <ul>
                                    {filterlist.map((item, index) => <li key={index} data-code={item.shortcode}
                                                                         onMouseDown={this.selectlanguage}>{item.name}</li>)}
                                </ul>
                                }
