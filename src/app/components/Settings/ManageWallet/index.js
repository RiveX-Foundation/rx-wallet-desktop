...
@inject((stores) => ({
  ...
  setSelectedWalletAddress: (address) =>
    stores.setting.setSelectedWalletAddress(address),
  ...
  RemoveMultiSigWallet: (publicaddress, cb) =>
    stores.walletStore.RemoveMultiSigWallet(publicaddress, cb),
  ExitMultiSigWallet: (publicaddress, cb) =>
    stores.walletStore.ExitMultiSigWallet(publicaddress, cb),
}))
@observer
class ManageWallet extends Component {
  ...
  handleRemoveWalletOk = () => {
    this.setState(
      ...
      () => {
        if (
          this.state.selectedremovewallet.isCloud &&
          this.state.selectedremovewallet.wallettype == "basicwallet"
        ) {
          this.props.RemoveMultiSigWallet(
            this.state.selectedwalletaddress,
            () => {
              this.props.removeWallet(this.state.selectedwalletaddress);
              createNotification(
                "success",
                intl.get("Settings.SuccessfullyRemoved")
              );
            }
          );
        }
        if (
          (!this.state.selectedremovewallet.isCloud &&
            this.state.selectedremovewallet.wallettype == "basicwallet") ||
          this.state.selectedremovewallet.wallettype == "hwwallet" ||
          this.state.selectedremovewallet.wallettype == "local"
        ) {
          this.props.removeWallet(this.state.selectedwalletaddress);
          createNotification(
            "success",
        ...
        if (
          this.state.selectedremovewallet.isOwner &&
          this.state.selectedremovewallet.wallettype == "sharedwallet"
        ) {
          this.props.RemoveMultiSigWallet(
            this.state.selectedwalletaddress,
            () => {
        ...
        <div className="centerpanel">
          ...
          >
            <p className="modalcontent">
              {intl.get("Modal.AreYouSureRemoveWallet", {
