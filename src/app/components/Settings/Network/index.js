...
@inject(stores => ({
  ...
  setNetwork : (code,networktype) => stores.network.setNetwork(code,networktype),
...
class Network extends Component {
  ...
  onChange = e => {
    if(e.currentTarget.getAttribute('data-networktype') == "eth"){
      this.setState({ethnetworkcode:this.props.ethnetworklist.find(x => x.name == e.target.value).shortcode});
    }

    if(e.currentTarget.getAttribute('data-networktype') == "wan"){
      this.setState({wannetworkcode:this.props.wannetworklist.find(x => x.name == e.target.value).shortcode});
    }
  }
  ...
  setfilterlist = e => {
    ...
    if(e.currentTarget.getAttribute('data-networktype') == "eth"){
      this.props.setNetwork(this.props.ethnetworklist.find(x => x.name == e.target.value).shortcode,"eth");
    ...
    if(e.currentTarget.getAttribute('data-networktype') == "wan"){
      this.props.setNetwork(this.props.wannetworklist.find(x => x.name == e.target.value).shortcode,"wan");
  ...
  hidelist = e => {
    //e.preventDefault();
    //e.stopPropagation();
    this.setState({ethautoliststyle : "autolisthide",wanautoliststyle : "autolisthide"});
  }
  ...
  handleOk = () => {
    this.setState({
      ...
      this.setState({ethnetworkcode:this.state.temporaryethnewnetworkcode,wannetworkcode:this.state.temporarywannewnetworkcode}, () => {
  ...
  render() {
    const {ethautoliststyle,ethfilterlist,ethnetworkcode,wanautoliststyle,wanfilterlist,wannetworkcode} = this.state;
    return (
      <div className="networkpanel fadeInAnim">
        <div className="title" ><span>{intl.get('Settings.ETHNetwork')}</span></div>
        <div className="centerpanel">
          <div className="inputwrapper">
            <div className="panelwrapper">
              <Input data-networktype="eth" value={this.props.selectedethnetwork.name } placeholder={intl.get('Settings.ETHNetwork')} onClick={this.showlist} onBlur={this.hidelist} onFocus={this.showlist} className="inputTransparent" onChange={this.setfilterlist} />
              <div className={ethautoliststyle}>
                {
                  ethfilterlist.length > 0 && 
                  <ul>
                    {ethfilterlist.map((item, index) => <li data-networktype="eth" key={index} data-code={item.shortcode} onMouseDown={this.selectnetwork} style={{color:item.color}}>{item.name}</li>)}
                  </ul>
                }
        ...
        <div className="title" ><span>{intl.get('Settings.WANNetwork')}</span></div>
        <div className="centerpanel">
          <div className="inputwrapper">
            <div className="panelwrapper">
              <Input data-networktype="wan" value={this.props.selectedwannetwork.name } placeholder={intl.get('Settings.WANNetwork')} onClick={this.showlist} onBlur={this.hidelist} onFocus={this.showlist} className="inputTransparent" onChange={this.setfilterlist} />
              <div className={wanautoliststyle}>
                {
                  wanfilterlist.length > 0 && 
                  <ul>
                    {wanfilterlist.map((item, index) => <li data-networktype="wan" key={index} data-code={item.shortcode} onMouseDown={this.selectnetwork} style={{color:item.color}}>{item.name}</li>)}
                  </ul>
                }
