...
class TokenTransferConfirmation extends Component {
  ...
  async componentDidMount() {
    ...
    if (
      this.props.selectedTokenAsset.TokenType == "eth" ||
      this.props.selectedTokenAsset.TokenType == "erc20"
    ) {
      this.web3Provider =
        this.props.selectedethnetwork.infuraendpoint +
        "/v3/c941387bd4d8467285c24d75ad3574a4";
    }

    this.props.setotptransfertoken("");
    this.get12SeedPhase();
    if (
      this.props.selectedTokenAsset.TokenType == "wan" ||
      this.props.selectedTokenAsset.TokenType == "wrc20"
    ) {
      this.setState(
        {
          gaspricevalue: 1,
  ...
  transfer = async () => {
    ...
    if (this.props.mfaenabled) {
      const secretAscii = base32.decode(
        this.props.decrypt(localStorage.getItem("twofasecret"))
      );
      const secretHex = this._toHex(secretAscii);
    ...
    if (
      ...
      this.state.otp != this.props.otptransfertoken
    ) {
      createNotification("error", intl.get("Error.InvalidOTP"));
      return;
    }
    ...
    if (
      ...
      !this.check2FAValid(this.props.googleAuthKey, this.state.otp)
    ) {
    ...
    if (
      ...
      this.props.twoFAPassword != this.state.otp
    ) {
      createNotification("error", intl.get("Error.Invalid2FAPassword"));
      return;
    }

    if (this.props.selectedwallet.wallettype == "sharedwallet") {
      //PROPOSE TO CLOUD
      this.props.wsCreateTrx(
        this.props.selectedwallet.publicaddress,
        this.props.tokentransferreceiver,
        this.props.tokentransfertoken
      );
    } else {
      //DIRECT EXECUTE TRX
      ...
      if (this.props.selectedTokenAsset.TokenType == "eth") {
        ...
        var sendamount = new BigNumber(
          web3.utils.toWei(this.props.tokentransfertoken.toString(), "ether")
        );
        let gasPrices = await this.getCurrentGasPrices();
        var tosend;
        var nonce = 0;
        ...
        web3.eth.estimateGas(tx).then((gaslimit) => {
         ...
         let compare = totalcost.comparedTo(totalbalance);
          ...
          console.log(
            "Sending: " + web3.utils.fromWei(tosend.toString(), "ether")
          );
          ...
          web3.eth.getTransactionCount(from).then((txCount) => {
            ...
            transaction.sign(
              Buffer.from(this.props.selectedTokenAsset.PrivateAddress, "hex")
            );
        ...
        const web3 = new Web3(this.web3Provider);
        var totalbalance = web3.utils.toWei(
          this.props.selectedTokenAsset.TokenBalance.toString(),
          "ether"
        );
        ...
        var sendingamount = new BigNumber(
          web3.utils.toWei(this.props.tokentransfertoken.toString(), "ether")
        );
        var TokenInfo = this.props.selectedTokenAsset.TokenInfoList[0];
        var abiArray = JSON.parse(TokenInfo.AbiArray);
        var receiver = this.props.tokentransferreceiver
          .toString()
          .toLowerCase(); //"0x8859C2BE1a9D6Fbe37E1Ed58c103487eE7B8b90F";
        var tosend;
        var tx = {
          from: from,
          to: receiver,
        ...
        iWanUtils
          .getNonce("WAN", this.props.selectedTokenAsset.PublicAddress)
          .then(async (res) => {
                ...
                iWanUtils
                  ...
                  .then(async (gas) => {
                    ...
                    var privKey = new Buffer(
                      this.props.selectedTokenAsset.PrivateAddress,
                      "hex"
        ...
        if (tokenitem.AssetCode == "WBTC") {
          ...
          var sendamount =
            parseFloat(this.props.tokentransfertoken.toString()) * 10 ** 8;
        } else if (tokenitem.AssetCode == "WUSDT") {
          console.log("SETTING WUSDT AMOUNT");
          var sendamount =
            parseFloat(this.props.tokentransfertoken.toString()) * 10 ** 6;
        } else if (tokenitem.AssetCode == "WEOS") {
          var sendamount =
            parseFloat(this.props.tokentransfertoken.toString()) * 10 ** 4;
        } else {
          var sendamount = web3.utils.toWei(
            this.props.tokentransfertoken.toString(),
            "ether"
        ...
        // var TokenInfo = this.props.selectedTokenAsset.TokenInfoList[0];
        var CurrentNetworkAllTokenInfo = toJS(this.props.allTokenAsset).find(
          (x) => x.AssetCode == this.props.selectedTokenAsset.AssetCode
        );
        ...
        try {
          contractdata.methods
            .transfer(this.props.tokentransferreceiver, sendamount.toString())
            .estimateGas({ from: this.props.selectedTokenAsset.PublicAddress })
            .then((gaslimit) => {
              console.log(gaslimit);
              var gas = new BigNumber(gaslimit);
              var gasPrice = web3.utils.toWei(
              ...
              if (
                parseFloat(web3.utils.fromWei(txcost.toString(), "ether")) >
                parseFloat(tokenitem.TokenBalance.toString())
              ) {
      ...
      <div className="tokentransferconfirmationpanel fadeInAnim">
        <div className="title">
          ...
          <span style={{ marginLeft: "20px" }}>
            {intl.get("TokenTransfer.Confirmation")}
          </span>
        </div>
          ...
          <center>
            ...
            >
              <div className="panellabel">
                {intl.get("TokenTransferConfirmation.From")}
              </div>
            ...
            >
              <div className="spacebetween">
                <div className="panellabel">
                  {intl.get("TokenTransferConfirmation.To")}
                </div>
                <div className="panelvalue">
                  {this.props.tokentransferreceiver}
                </div>
            ...
            >
              ...
              {this.props.selectedTokenAsset.TokenType == "erc20" && (
                <React.Fragment>
                  <div className="spacebetween" style={{ marginTop: "10px" }}>
                    <div className="panellabel" style={{ marginTop: "10px" }}>
                      Gas limit (leave empty for auto)
                    </div>
                    <Input
                  ...
                  <div className="spacebetween" style={{ marginTop: "10px" }}>
                    <div className="panellabel">
                      If transactions are failing, try setting a higher has
                      limit (eg. 120000)
                    </div>
                  </div>
                </React.Fragment>
            ...
            >
              {this.props.twoFAType == "sms" && (
                <React.Fragment>
                  ...
                  >
                    <div className="panellabel" style={{ paddingLeft: "0px" }}>
                      <Input
                        className="inputTransparent otpinputclass"
                        onChange={this.OTPChange}
                        placeholder={intl.get("Auth.EnterOTP")}
                      />
                    </div>
                  </div>
                  <div className="panelvalue" style={{ paddingRight: "0px" }}>
                    <Button className="radiusbutton" onClick={this.requestotp}>
                      {intl.get("Auth.RequestOTP")}
                    </Button>
                  </div>
                </React.Fragment>
              )}
              ...
              {this.props.twoFAType == "password" && (
                <div className="panelwrapper borderradiusfull spacebetween">
                  <div className="panellabel" style={{ marginTop: "5px" }}>
                ...
                <React.Fragment>
                  ...
                  <div style={{ marginTop: "40px" }}>
                    <div style={{ width: "400px" }}>
                      {intl.get("Settings.Key") +
                        " : " +
                        this.props.googleAuthKey}
                    </div>
                    <div
                      style={{
                        marginTop: "20px",
                    ...
                    >
                      <Input
                        ...
                        placeholder={intl.get(
                          "Settings.2FASecurity.SecurityCode"
                        )}
                      />
                    </div>
                  </div>
