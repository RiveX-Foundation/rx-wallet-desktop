...
import './index.less';
import {createNotification, getDatefromTimestamp, numberWithCommas, toFixedNoRounding} from 'utils/helper';
            ...
            <div className="walletdetailpanel fadeInAnim">
                ...
                <div>
                    ...
                    <div className="contentpanel">
                        ...
                        <div
                            className="rvxbalance">{this.props.selectedTokenAsset.TokenBalance ? `${this.props.selectedTokenAsset.TokenBalance % 1 != 0 ? toFixedNoRounding(this.props.selectedTokenAsset.TokenBalance, 4) : toFixedNoRounding(this.props.selectedTokenAsset.TokenBalance, 2)}` : `0.00`}<span>{this.props.selectedTokenAsset.AssetCode.toUpperCase()}</span>
                        </div>
                        <div
                            className="usdbalance">${numberWithCommas(parseFloat(!isNaN(this.props.selectedTokenAsset.TokenPrice * this.props.selectedTokenAsset.TokenBalance) ? this.props.selectedTokenAsset.TokenPrice * this.props.selectedTokenAsset.TokenBalance : 0), true)} {this.props.currencycode}</div>
                    ...
                    <Tooltip cursor={false} />  */}
                                    <Curve type={"natural"}/>
                                    <Area type="monotone" dataKey="value" baseLine={5} stroke="rgb(100, 244, 244)"
                                          fillOpacity={1} fill="url(#gradient)"/>
                                </AreaChart>
                            </ResponsiveContainer>
                            <div className="chartvalue-high">{dataMax}</div>
                            <div className="chartvalue-low">{dataMin}</div>
                        </div>
                        <Button className="butreceive button" onClick={this.receiveToken}
                                style={{marginRight: "20px", marginTop: "35px"}}><span><img src={buttonreceive}/></span><span
                            style={{marginLeft: "10px"}}>{intl.get('Token.Receive')}</span></Button>
                        <Button className="butsend button" onClick={this.transferToken}><span><img
                            src={buttonsend}/></span><span style={{marginLeft: "10px"}}>{intl.get('Token.Send')}</span></Button>
                    </div>
                    {
                        this.props.gettrxlist.length > 0 ?
                            <div className="tablewrapperctn">
                                <div
                                    className="subtitle">{intl.get('Transaction.TransactionHistory').toUpperCase()}</div>
                                <div className="tablewrapper">
                                    <table>
                                        ...
                                        <tr className="hovernone">
                                            ...
                                            {(this.props.selectedTokenAsset.TokenType != "wrc20" && this.props.selectedTokenAsset.TokenType != "wan") &&
                                            ...
                                            this.props.gettrxlist.map((item, i) => {
                                                    ...
                                                    return (
                                                        <tr
                                                            ...
                                                            <td className="tdcenter tdgrey"><span
                                                                className="tdellipsis">{item.from}</span></td>
                                                            <td className="tdcenter tdgrey"><span
                                                                className="tdellipsis">{item.to}</span></td>
                                                            {(this.props.selectedTokenAsset.TokenType != "wrc20" && this.props.selectedTokenAsset.TokenType != "wan") &&
