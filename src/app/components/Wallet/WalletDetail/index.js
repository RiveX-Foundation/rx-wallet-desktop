...
@inject((stores) => ({
  ...
  removeTokenAssetFromWallet: (token) =>
    stores.walletStore.removeTokenAssetFromWallet(token),
  wsGetMultiSigTrx: (walletpublicaddress, senderpublicaddress, assetcode) =>
    stores.walletStore.wsGetMultiSigTrx(
      walletpublicaddress,
      senderpublicaddress,
      assetcode
    ),
  ...
  settrxdetail: (
    ...
    stores.walletStore.settrxdetail(
      block,
      hash,
      from,
      to,
  ...
  RemoveTokenAssetInCloudWallet: (cb) =>
    stores.walletStore.RemoveTokenAssetInCloudWallet(cb),
}))
@observer
class WalletDetail extends Component {
  ...
  loadTransaction = () => {
    this.props.LoadTransactionByAddress(
      this.props.selectedTokenAsset.PublicAddress
    );
  };

  transferToken = () => {
    var balance = this.props.selectedTokenAsset.TokenBalance
      ? `${
          this.props.selectedTokenAsset.TokenBalance % 1 != 0
            ? toFixedNoRounding(this.props.selectedTokenAsset.TokenBalance, 4)
            : toFixedNoRounding(this.props.selectedTokenAsset.TokenBalance, 2)
        }`
      : `0.00`;
    if (balance == "0.00") {
  ...
  loadtrxdetail = (event) => {
    if (
      ...
      iWanUtils
        .getBlockByNumber("WAN", event.currentTarget.dataset.block)
        .then((response) => {
  ...
  confirmRemoveToken = () => {
    ...
    createNotification(
      "success",
      intl.get("Wallet.RemovedTokenAsset", {
        code: this.props.selectedTokenAsset.AssetCode.toUpperCase(),
      })
    );
    this.props.setCurrent("selectedwallet");
  };
  ...
  render() {
    var sparkline = this.props.TokenSparkLine.find(
      (x) => x.AssetCode == this.props.selectedTokenAsset.AssetCode
    );
    if (sparkline != null) sparkline = sparkline.sparkline;
    const dataMax =
      sparkline != null ? Math.max(...sparkline.map((i) => i.value)) : 0;
    const dataMin =
      sparkline != null ? Math.min(...sparkline.map((i) => i.value)) : 0;
    return (
      <div className="walletdetailpanel fadeInAnim">
            ...
            <div className="contentpanel">
              ...
              <div className="usdbalance">
                ...
                {numberWithCommas(
                  parseFloat(
                    !isNaN(
                      this.props.selectedTokenAsset.TokenPrice *
                        this.props.selectedTokenAsset.TokenBalance
                    )
                      ? this.props.selectedTokenAsset.TokenPrice *
                          this.props.selectedTokenAsset.TokenBalance
                  ...
                  <AreaChart data={sparkline} baseValue={dataMin}>
                    <defs>
                      <linearGradient id="gradient" x1="0" y1="0" x2="0" y2="1">
                        <stop
                          offset="5%"
                          stopColor="rgb(100, 244, 244)"
                          stopOpacity={0.7}
                        />
                        <stop
                          offset="95%"
                          stopColor="rgb(28, 31, 70)"
                          stopOpacity={0}
                        />
                      </linearGradient>
                    </defs>
              ...
              >
                ...
                <span style={{ marginLeft: "10px" }}>
                  {intl.get("Token.Receive")}
                </span>
              </Button>
              <Button className="butsend button" onClick={this.transferToken}>
                <span>
                  <img src={buttonsend} />
                </span>
                <span style={{ marginLeft: "10px" }}>
            ...
            {this.props.gettrxlist.length > 0 ? (
              <div className="tablewrapperctn">
                <div className="subtitle">
                  {intl.get("Transaction.TransactionHistory").toUpperCase()}
                </div>
                <div className="tablewrapper">
                  <table>
                    <thead>
                      <tr className="hovernone">
                        <td className="tdcenter header">
                          {intl.get("Table.Status")}
                        </td>
                        <td className="tdright header">
                          {intl.get("Table.Amount")}
                        </td>
                        <td className="tdcenter header">
                          {intl.get("Table.From")}
                        </td>
                    ...
                    <tbody>
                      {this.props.gettrxlist.map((item, i) => {
                        ...
                        if (item.value != 0) {
                          if (
                            this.props.selectedTokenAsset.PublicAddress.toLowerCase() ==
                        ...
                        return (
                          ...
                          >
                            <td className="tdcenter tdgrey">
                              {intl.get("STATUS." + item.status)}
                            </td>
                            <td className={valueClass}>
                              {plusminusSign} {item.value}{" "}
                              {this.props.selectedTokenAsset.AssetCode.toUpperCase()}
                            </td>
                            <td className="tdcenter tdgrey">
                              <span className="tdellipsis">{item.from}</span>
                            </td>
                            <td className="tdcenter tdgrey">
                              <span className="tdellipsis">{item.to}</span>
                            </td>
              ...
              <div className="notransaction">
                {intl.get("Transaction.NoTransaction")}
              </div>
            )}
          </div>
        )}
        ...
        >
          <p className="modalcontent">
            {intl.get("Modal.AreYouSureRemoveTokenAsset")}
          </p>
