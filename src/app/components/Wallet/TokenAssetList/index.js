...
@inject((stores) => ({
  ...
  addTokenAssetToWallet: (token) =>
    stores.walletStore.addTokenAssetToWallet(token),
  GenerateBIP39Address: async (derivepath, seed) =>
    stores.walletStore.GenerateBIP39Address(derivepath, seed),
  ...
  InsertTokenAssetToCloudWallet: (tokenasset, publicaddress, cb) =>
    stores.walletStore.InsertTokenAssetToCloudWallet(
      tokenasset,
      publicaddress,
      cb
    ),
  decrypt: (text) => stores.walletStore.decrypt(text),
  GenerateAddressByPrivateKey: async (privatekey) =>
    stores.walletStore.GenerateAddressByPrivateKey(privatekey),
}))
@observer
class TokenAssetList extends Component {
  state = {
    color:
      ...
      this.props.selectedTokenAsset.TokenType == "wan"
        ? this.props.selectedwannetwork.color
        : this.props.selectedethnetwork.color,
    name:
      this.props.selectedTokenAsset.TokenType == "wrc20" ||
      this.props.selectedTokenAsset.TokenType == "wan"
        ? this.props.selectedwannetwork.name
        : this.props.selectedethnetwork.name,
  };
  ...
  addTokenToWallet = async (tokenasset) => {
    ...
    if (
      this.props.selectedWallet.tokenassetlist.some(
        (x) => x.AssetCode == tokenasset.AssetCode
      )
    ) {
      ...
      if (
        this.props.selectedWallet.seedphase == "null" ||
        this.props.selectedWallet.seedphase == null ||
        this.props.selectedTokenAsset.seedphase == ""
      ) {
        var walletkey = await this.props.GenerateAddressByPrivateKey(
          this.props.selectedWallet.privatekey
        );
        ...
        var walletkey = await this.props.GenerateBIP39Address(
          derivepath + "0",
          seed
        );
      ...
      if (tokenasset.TokenType == "wan" || tokenasset.TokenType == "wrc20") {
      ...
      if (this.props.selectedWallet.isCloud) {
        this.props.InsertTokenAssetToCloudWallet(
          [tokenasset],
        ...
        <div>
          ...
          >
            ...
            <span style={{ marginLeft: "20px" }}>
              {intl.get("Wallet.AddNewTokenAsset").toUpperCase()}
            </span>
          </div>
