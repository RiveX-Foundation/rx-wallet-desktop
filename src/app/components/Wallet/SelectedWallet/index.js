...
@inject((stores) => ({
  ...
  setselectedTokenAsset: (tokenasset) =>
    stores.walletStore.setselectedTokenAsset(tokenasset),
  ...
  GetPrimaryTokenAssetByNetwork: () =>
    stores.walletStore.GetPrimaryTokenAssetByNetwork(),
  GetAllTokenAssetByNetwork: () =>
    stores.walletStore.GetAllTokenAssetByNetwork(),
...
class SelectedWallet extends Component {
  ...
  goToAssetTokenList = () => {
    if (
      this.props.selectedwallettype == "basicwallet" ||
      this.props.selectedwallettype == "sharedwallet" ||
      this.props.selectedwallettype == "local"
    ) {
        ...
        {this.props.selectedWallet.walletname != null && (
          <div>
                ...
                <div className="totalworth">
                  {
                    <div className="amount">
                      {this.props.getTotalWorth(this.props.selectedWallet)}
                    </div>
                  }
            ...
            {w.wallettype == "sharedwallet" && (
              <center>
                <div onClick={this.receiveToken} className="sharedwalletpanel">
                  {w.totalsignatures} / {w.totalowners}
                </div>
              </center>
            )}
            ...
            <div className="tokenwrapper">
              {this.props.selectedWallet.tokenassetlist.map((item, index) => {
                var sparkline = this.props.TokenSparkLine.find(
                  (x) => x.AssetCode == item.AssetCode
                );
                if (sparkline != null) sparkline = sparkline.sparkline;
                const dataMax =
                  sparkline != null
                    ? Math.max(...sparkline.map((i) => i.value))
                    : 0;
                const dataMin =
                  sparkline != null
                    ? Math.min(...sparkline.map((i) => i.value))
                    : 0;
                return (
                  <div
                    key={index}
                    ...
                    <div className="tokenassetitemrow">
                      ...
                      <div className="infoctn">
                        ...
                        {this.props.selectedWallet.wallettype != "hwwallet" ? (
                          ...
                          <div className="assetcodename">
                            {item.Name} - <span>{item.PublicAddress}</span>
                          </div>
                        )}
                      </div>
                    </div>
                    <div className="tokenassetitemrow">
                      <div className="amountctn">
                        <div className="totalcoin">
                          {item.TokenBalance
                            ? `${
                                item.TokenBalance % 1 != 0
                                  ? toFixedNoRounding(item.TokenBalance, 4)
                                  : toFixedNoRounding(item.TokenBalance, 2)
                              }`
                            : `0.00`}
                          <span>{item.AssetCode.toUpperCase()}</span>
                        </div>
                        {this.props.currencycode == "USD" && (
                          <div className="totalcurrency">
                            ...
                            {numberWithCommas(
                              parseFloat(
                                !isNaN(item.TokenPrice * item.TokenBalance)
                                  ? item.TokenPrice * item.TokenBalance
                                  : 0
                              ),
                              true
                            )}{" "}
                      ...
                      <div className="buttonctn">
                        <div className="btnitem">
                          <img
                            src={buttonreceive}
                            onClick={(e) => this.receiveToken(e, item)}
                          />
                        </div>
                        <div className="btnitem">
                          <img
                            src={buttonsend}
                            onClick={(e) => this.transferToken(e, item)}
                          />
                        </div>
                      </div>
                    </div>
