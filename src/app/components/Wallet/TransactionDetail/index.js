...
@inject((stores) => ({
  ...
  wsApproveMultiSigTrx: (trxid, to, total, isexecute) =>
    stores.walletStore.wsApproveMultiSigTrx(trxid, to, total, isexecute),
...
class TransactionDetail extends Component {
    ...
    if (this.props.selectedwallet.wallettype == "basicwallet") {
      layout = (
        <div>
          ...
          >
            <div className="spacebetween" style={{ marginBottom: "10px" }}>
              ...
              <div className="panelvalue">
                {this.props.trxdetail.value}{" "}
                {this.props.selectedTokenAsset.TokenType.toString().toUpperCase()}
              </div>
            </div>
          ...
          >
            <div className="panellabel">
              {intl.get("Transaction.CreatedOn")}
            </div>
            <div className="panelvalue">
              {dateobj.getFullYear() +
                "-" +
          ...
          >
            <div className="panellabel">
              {intl.get("Transaction.Confirmation")}
            </div>
            <div className="panelvalue">
              {this.props.trxdetail.confirmation}
            </div>
          </div>
        </div>
      );
      ...
      layout = (
        <div>
          ...
          >
            <div className="panellabel">
              {intl.get("Transaction.TransactionID")}
            </div>
            <div className="panelvalue">{this.props.trxdetail.hash}</div>
          </div>
          <div
          ...
          <div className="spacebetween" style={{ width: "700px" }}>
            <div
              className="panelwrapper borderradiusfull spacebetween signerspanelcolor"
              style={{
                marginBottom: "10px",
                padding: "10px 10px",
                width: "500px",
            ...
            >
              <div className="panellabel">
                {intl.get("Transaction.TotalSigners")}
              </div>
              <div className="panelvalue">
                {this.props.trxdetail.signers.length} /{" "}
                {this.props.selectedwallet.totalsignatures}
              </div>
            </div>
            ...
            <div>
              {this.props.trxdetail.action == "approve" && (
                <Button
                  ...
                  data-trxid={this.props.trxdetail.hash}
                >
                  {intl.get("Transaction.Approve")}
                </Button>
              )}

              {this.props.trxdetail.action != "approve" && (
                <div className="actionlabel">
                  {intl.get("STATUS." + this.props.trxdetail.action)}
                </div>
              )}
            </div>
          </div>
  ...
  approve = (e) => {
    ...
    this.props.wsApproveMultiSigTrx(
      hash,
      this.props.trxdetail.to,
      this.props.trxdetail.value,
      isexecute
    );
    console.log(hash);
        ...
        <div className="content">
          <center>
            ...
            {this.props.selectedwallet.wallettype == "sharedwallet" && (
              <div className="spacebetween" style={{ width: "700px" }}>
                <div
                  className="panelwrapper borderradiusfull signerspanelcolor"
                  style={{
                    marginBottom: "10px",
                    padding: "10px",
                    width: "700px",
