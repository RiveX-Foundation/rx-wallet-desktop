...
@inject((stores) => ({
  ...
  CreateHWWallet: (
    ...
    stores.walletStore.CreateHWWallet(
      walletname,
      publicaddress,
      derivepath,
      tokentype,
...
class HWWalletDetail extends Component {
  ...
  UNSAFE_componentWillReceiveProps(newProps) {
    if (this.state.createhwwalletcb) {
      createNotification(
        "success",
        intl.get("Wallet.AddedNewAssetToken", {
          code: this.state.newaddedaddress,
        })
      );
  ...
  deriveAddresses = (start, limit, visible = false) => {
    ...
    hdKeys.forEach((hdKey) => {
      ...
      let isUsed =
        JSON.stringify(this.props.wallets).indexOf(checkedAddress) > -1;
      addresses.push({
        key: hdKey.address,
      ...
      if (isUsed) {
        web3.eth.getBalance(checkedAddress).then((balance) => {
          ...
          var balance = balance
            ? `${
                balance % 1 != 0
                  ? toFixedNoRounding(balance, 4)
                  : toFixedNoRounding(balance, 2)
              }`
            : `0.00`;
  ...
  addAddress = (e, item) => {
    this.setState(
      {
        currentaddedaddress: this.state.currentaddedaddress.concat(
          item.address
        ),
  ...
  updateTokenAssetList = (walletlist, selectedwalletindex) => {
    this.setState(
      ...
      () => {
        var tokenitem = toJS(this.props.allTokenAsset).find(
          (x) => x.AssetCode == "eth"
        );
        // var tokenitem = {};
      ...
      if (value !== null) {
        ...
        if (walletlist.length > 0) {
          walletlist.map((wallet, index) => {
            if (
              wallet.publicaddress == this.props.selectedWallet.publicaddress
            ) {
              createNotification(
                "success",
                intl.get("Wallet.AddedNewAssetToken", {
                  code: tokenasset.AssetCode.toUpperCase(),
                })
              );
        ...
        <div>
          ...
          >
            ...
            <span style={{ marginLeft: "20px" }}>
              ...
              <span style={{ color: this.props.selectednetwork.color }}>
                {this.props.selectednetwork.name}
              </span>
            </span>
          </div>
          <div className="tokenassetwrapper" style={{ margin: "20px" }}>
            {this.state.allAddress.map((item, index) => {
              ...
              if (
                item.used &&
                this.state.calcbalanceaddress.find(
              ...
              return (
                <div key={index} className="tokenassetitem">
                  <div className="tokenassetitemrow_hw">
                    ...
                    {!item.used &&
                    JSON.stringify(this.state.currentaddedaddress).indexOf(
                      item.address
                    ) == -1 ? (
                      <Button
                        type="primary"
                        onClick={(e) => this.addAddress(e, item)}
                      >
                        {intl.get("Wallet.Add")}
                      </Button>
                    ) : (
