...
class Setting {
  ...
  update2FA(info) {
    ...
    if (
      this.userstore.googleAuthKey == null ||
      this.userstore.googleAuthKey == ""
    ) {
      bodyFormData.set(
        "googleAuthKey",
    ...
    axios({
      ...
      .then(async function (response) {
        if (response.data.status == 200) {
          ...
          var info = {
            twoFAType: response.data.user.TwoFAType,
            twoFAPassword: response.data.user.TwoFAPassword,
            googleAuthKey: response.data.user.GoogleAuthKey,
          };
