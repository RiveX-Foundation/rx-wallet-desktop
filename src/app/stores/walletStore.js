...
class walletStore {
  ...
  @computed get selectedwalletlist() {
    ...
    var walletlist = this.walletlist.filter(
      (x) => x.wallettype == this.selectedwallettype
    );
  ...
  @computed get gettrxlist() {
    ...
    this.trxlist.map((item, i) => {
      if (item.to != "") {
        var trx = {
          ...
          gasprice:
            this.selectedTokenAsset.TokenType == "wrc20" ||
            this.selectedTokenAsset.TokenType == "wan"
              ? item.gasPrice / 1000000000 + " gwin"
              : item.gasPrice / 1000000000 + " gwei",
          gasused:
            this.selectedTokenAsset.TokenType == "wrc20" ||
            this.selectedTokenAsset.TokenType == "wan"
              ? item.gas / 1000000000 + " gwin"
    ...
    this.multisigtrxlist.map((item, i) => {
      ...
      if (signers.find((x) => x.UserId == this.userstore.userid) != null) {
        //SIGNED
        action = "approved";
      }
  ...
  @computed get TotalWorthMYR() {
    ...
    return `$${numberWithCommas(
      parseFloat(
        !isNaN(this.totalassetworth * this.convertrate)
          ? this.totalassetworth * this.convertrate
          : 0
      ),
      true
    )}`;
  ...
  @computed get TotalWorthCNY() {
    ...
    return `$${numberWithCommas(
      parseFloat(
        !isNaN(this.totalassetworth * this.convertratecny)
          ? this.totalassetworth * this.convertratecny
          : 0
      ),
      true
    )}`;
  ...
  async removeTokenAssetFromWallet(token) {
    var itemindex = this.selectedwallet.tokenassetlist.findIndex(
      (x) =>
        x.PublicAddress == token.PublicAddress && x.AssetCode == token.AssetCode
    );
  ...
  @action setSelectedWallet(publicaddress) {
    this.selectedwallet = this.walletlist.find(
      (x) => x.publicaddress == publicaddress
    );
  ...
  async LoadTransactionByAddress(addr) {
    if (this.selectedwallet.wallettype == "sharedwallet") {
      this.wsGetMultiSigTrx(
        this.selectedwallet.publicaddress,
        this.selectedTokenAsset.PublicAddress,
        this.selectedTokenAsset.AssetCode
      );
      /*
      ...
      axios({
        method: "get",
        url:
          this.networkstore.selectedethnetwork.etherscanendpoint +
  ...
  @action changeWalletName(walletpublicaddress, newwalletname) {
    let wallet = this.walletlist.find(
      (x) => x.publicaddress == walletpublicaddress
    );
  ...
  @action removeWallet(publicaddress) {
    ...
    const filterwalletlist = this.walletlist.filter(
      (x) => x.publicaddress !== publicaddress
    );
    ...
    axios({
      ...
      .then(function (response) {
        ...
        response.data.wallet.map((walletmodel, index) => {
          ...
          if (
            that.walletlist.find(
              (x) =>
                x.publicaddress == walletitem.publicaddress &&
                x.wallettype == walletitem.wallettype
            ) == null
          ) {
  ...
  async SaveWallet(
    ...
    var wallet = {
      ...
      addresstype: addresstype,
      wallettype: wallettype,
      tokenassetlist: await this.insertPrimaryAssetTokenList(
  ...
  insertPrimaryAssetTokenList = async (
    seedphase,
    iscloud,
    ...
    if (self.primaryTokenAsset.length > 0) {
      var promises = self.primaryTokenAsset.map(async (tokenitem, index) => {
        ...
        if (seedphase != "") {
          var walletkey = await this.GenerateBIP39Address(
            derivepath + "0",
            seedphase
          );
        ...
        if (tokenitem.TokenType == "wan" || tokenitem.TokenType == "wrc20") {
          //IF WANCHAIN . CONVERT ALL ADDRESS TO LOWERCASE
  ...
  encrypt(text, pass) {
    ...
    if (
      ...
      JSON.parse(localStorage.getItem("iv") == null)
    ) {
  ...
  async CreateEthAddressByPrivateKey() {
    try {
      ...
      var walletkey = await this.GenerateAddressByPrivateKey(
        this.restoreprivatekey
      );
  ...
  generate12SeedPhase = () => {
    ...
    if (languageIntl.language == "zh_CN")
      selectedwordlist = bip39.wordlists.chinese_simplified;
    if (languageIntl.language == "zh_TW")
      selectedwordlist = bip39.wordlists.chinese_traditional;
    ...
    axios({
      ...
      .then(function (response) {
        ...
        if (response.data.status == 200) {
          self.wsGetMultiSigTrx(
            fromwalletpublicaddress,
            self.selectedTokenAsset.PublicAddress,
            self.selectedTokenAsset.AssetCode
          );
  ...
  async wsApproveMultiSigTrx(trxid, to, total, isexecute) {
    ...
    if (isexecute) {
      ...
      if (this.selectedTokenAsset.TokenType == "erc20") {
        ...
        var contractdata = new web3.eth.Contract(
          abiArray,
          TokenInfo.ContractAddress
        ); //, {from: this.props.selectedwallet.publicaddress}); //).at(this.tokencontract);
        var rawTransaction = {};
        try {
          rawTransaction = {
            ...
            data: contractdata.methods
              .transfer(to, web3.utils.toWei(total, "ether"))
              .encodeABI(), //contract.transfer.getData(this.tokencontract, 10, {from: this.props.selectedwallet.publicaddress}),
          ...
          var privKey = new Buffer(
            this.selectedTokenAsset.PrivateAddress,
            "hex"
          );
        ...
        iWanUtils
          .getNonce("WAN", this.selectedTokenAsset.PublicAddress)
          .then(async (res) => {
                ...
                iWanUtils
                  ...
                  .then(async (gas) => {
                    ...
                    var privKey = new Buffer(
                      this.selectedTokenAsset.PrivateAddress,
                      "hex"
                    ); //"35e0ec8f5d689f370cdc9c35a04d1664c9316aadbd2ac508cfa69f3de7aaa233", 'hex');
        ...
        web3.eth.getTransactionCount(from).then((txCount) => {
          ...
          var transaction =
            this.networkstore.selectedethnetwork.shortcode == "mainnet"
              ? new Tx(details)
              : new Tx(details, {
          ...
          transaction.sign(
            Buffer.from(this.selectedTokenAsset.PrivateAddress, "hex")
          );
  ...
  CreateBasicWalletOnCloud = (cb) => {
    ...
    bodyFormData.append(
      "network",
      this.networkstore.selectedethnetwork.shortcode
    );
  ...
  async wsCreateSharedWallet() {
    ...
    bodyFormData.set("holders", [
      { UserId: this.userstore.userid, UserName: this.userstore.name },
    ]);
    ...
    axios({
      ...
      .then(async (response) => {
        ...
        //handle success
        var tokenassetlist = await that.insertPrimaryAssetTokenList(
          that.seedphaseinstring,
          true,
        ...
        that.InsertTokenAssetToCloudWallet(
          tokenassetlist,
          wallet.PublicAddress,
          function () {}
        );
      ...
      .then(function (response) {
        ...
        if (response.data.status == 200) {
          ...
          self.SaveWallet(
            ...
            wallet.WalletName,
            wallet.Seedphase,
            wallet.PrivateAddress,
            wallet.DerivePath,
            wallet.PublicAddress,
            wallet.AddressType,
            "sharedwallet",
            wallet.NumbersOfOwners,
            wallet.NumbersOfSigners,
            wallet.Holders
          );
  ...
  GetPrimaryTokenAssetByNetwork() {
    ...
    bodyFormData.set(
      "wannetwork",
      this.networkstore.selectedwannetwork.shortcode
    );
      ...
      if (tokenitem.TokenType == "eth") {
        ...
        web3.eth.getBalance(tokenitem.PublicAddress).then((balance) => {
          ...
          self.totalassetworth +=
            this.getTokenPrice(tokenitem.AssetCode) * tokenitem.TokenBalance;
        });
        ...
        var TokenInfo = tokenitem.TokenInfoList.find(
          (x) => x.Network == this.networkstore.selectedethnetwork.shortcode
        );
        ...
        web3.eth
          .call({
            ...
            data: contract.methods
              .balanceOf(tokenitem.PublicAddress)
              .encodeABI(),
          })
          .then((balance) => {
        ...
        var TokenInfo = tokenitem.TokenInfoList.find(
          (x) => x.Network == this.networkstore.selectedwannetwork.shortcode
        );
        TokenInfo = toJS(TokenInfo);
        var CurrentNetworkAllTokenInfo = toJS(this.allTokenAsset).find(
          (x) => x.AssetCode == tokenitem.AssetCode
        );
        ...
        console.log(TokenInfo);
        iWanUtils
          .getBalance("WAN", tokenitem.PublicAddress)
          .then((res) => {
            try {
              var balance = res;
              var tokenbal = new BigNumber(
  ...
  @action getTotalWorth(selectedWallet) {
    /*
        ...
        */
    return `$${numberWithCommas(
      parseFloat(!isNaN(this.totalassetworth) ? this.totalassetworth : 0),
      true
    )}`;
  }
  ...
  toMYR = async () => {
    ...
    let response = await axios.get(
      "http://api.openrates.io/latest?base=USD&symbols=MYR"
    );
  ...
  toCNY = async () => {
    ...
    let response = await axios.get(
      "http://api.openrates.io/latest?base=USD&symbols=CNY"
    );
