...
class WanAddress {
  ...
  @computed get historyList() {
    ...
    Object.keys(self.transHistory).forEach((item) => {
      if (addrList.includes(self.transHistory[item]["from"])) {
        ...
        let type = checkAddrType(
          self.transHistory[item]["from"],
          self.addrInfo
        );
        historyList.push({
          key: item,
          ...
          to: wanUtil.toChecksumAddress(
            self.transHistory[item].to.toLowerCase()
          ),
          value: formatNum(fromWei(self.transHistory[item].value)),
          status:
            languageIntl.language && ["Failed", "Success"].includes(status)
              ? intl.get(`TransHistory.${status.toLowerCase()}`)
              : intl.get("TransHistory.pending"),
  ...
  @computed get privateHistoryList() {
    ...
    Object.keys(self.transHistory).forEach((item) => {
      if (
        addrList.includes(self.transHistory[item]["from"]) &&
        self.transHistory[item].to === PrivateAddr
      ) {
        let status = self.transHistory[item].status;
        let type = checkAddrType(
          self.transHistory[item]["from"],
        ...
        historyList.push({
          ...
          time: timeFormat(self.transHistory[item]["sendTime"]),
          from: self.addrInfo[type][self.transHistory[item]["from"]].name,
          to: /refund/i.exec(operation)
            ? self.transHistory[item]["from"]
            : /send/i.exec(operation)
            ? self.transHistory[item]["privateTo"]
            : to,
          type: /send/i.exec(operation)
            ? intl.get("Common.send")
            : /refund/i.exec(operation)
            ? intl.get("TransHistory.refund")
            : "",
          value: /refund/i.exec(operation)
            ? formatNum(self.transHistory[item]["redeemAmount"] || 0)
            : formatNum(fromWei(self.transHistory[item].value)),
          status:
            languageIntl.language && ["Failed", "Success"].includes(status)
              ? intl.get(`TransHistory.${status.toLowerCase()}`)
              : intl.get("TransHistory.pending"),
  ...
  @computed get tokenTransferHistoryList() {
    ...
    Object.keys(self.transHistory).forEach((item) => {
      if (
        addrList.includes(self.transHistory[item]["from"]) &&
        self.transHistory[item].transferTo &&
        tokens.currTokenAddr.toLowerCase() ===
          self.transHistory[item].to.toLowerCase()
      ) {
        let status = self.transHistory[item].status;
        ...
        historyList.push({
          ...
          from: self.addrInfo[type][self.transHistory[item].from].name,
          to: wanUtil.toChecksumAddress(
            self.transHistory[item].transferTo.toLowerCase()
          ),
  ...
  @computed get addrSelectedList() {
    ...
    self.ledgerAddrList.forEach((addr) => {
      addrList.push("Ledger: " + addr.address);
    });
    self.trezorAddrList.forEach((addr) => {
      addrList.push("Trezor: " + addr.address);
    });
    return addrList;
  }
  ...
  @computed get getNormalAmount() {
    ...
    Object.values({
      normal: self.addrInfo.normal,
      import: self.addrInfo.import,
    }).forEach((value) => {
      sum = sum.plus(
        Object.values(value).reduce(
          (prev, curr) => new BigNumber(prev).plus(new BigNumber(curr.balance)),
          0
        )
      );
    });
    ...
    Object.values(self.addrInfo).forEach((value) => {
      sum = sum.plus(
        Object.values(value).reduce(
          (prev, curr) =>
            new BigNumber(prev)
              .plus(new BigNumber(curr.balance))
              .plus(
                isNaN(curr.wbalance)
                  ? new BigNumber(0)
                  : new BigNumber(curr.wbalance)
              ),
          0
        )
      );
    ...
    wand.request("transaction_showRecords", (err, val) => {
      if (!err && val.length !== 0) {
        ...
        val.forEach((item) => {
          ...
          if (
            item.txHash !== "" &&
            (item.txHash !== item.hashX || item.status === "Failed")
          ) {
  ...
  @action updateName(arr, wid) {
    ...
    wand.request(
      "account_update",
      {
        ...
        meta: {
          name: arr.name,
          addr: arr.address.toLowerCase(),
          waddr: arr.waddress ? arr.waddress.toLowerCase() : "",
        },
      },
      (err, val) => {
        if (!err && val) {
    ...
    switch (ver) {
      case "original":
        try {
          let noWaddressArr = [];
          let setWaddress = (obj) => {
            self.addrInfo[typeFunc(obj.id)][
              wanUtil.toChecksumAddress(obj.address)
                      ...
                      if (!err) {
                        ...
                        wand.request(
                          "account_update",
                          {
                            ...
                            meta: {
                              name: item.name,
                              addr: `0x${val.address.toLowerCase()}`,
                              waddr: `0x${val.waddress}`,
                            },
                          },
                          (err, res) => {
                            if (!err && res) {
