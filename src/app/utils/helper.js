...
export const getNewAccountName = function (chainID, prefix) {
  return new Promise((resolve, reject) => {
    wand.request(
      "address_getNewNameForNativeAccount",
      { chainID, prefix },
      (err, name) => {
        if (err) {
...
export const getNewPathIndex = function (chainID, pathForm, walletID) {
  return new Promise((resolve, reject) => {
    wand.request(
      "address_getNewPathIndex",
      { chainID, pathForm, walletID },
      (err, index) => {
        if (err) {
        ...
        if (!err) {
          ...
          wand.request(
            "account_create",
            {
              ...
              meta: {
                name,
                addr: `0x${val_address_get.address}`.toLowerCase(),
                waddr: `0x${val_address_get.waddress}`.toLowerCase(),
              },
            },
            (err, val_account_create) => {
              if (!err && val_account_create) {
                let addressInfo = {
                  start: index,
                  name,
                  path,
                  address: wanUtil.toChecksumAddress(
                    `0x${val_address_get.address}`
                  ),
                  waddress: wanUtil.toChecksumOTAddress(
                    `0x${val_address_get.waddress}`
                  ),
                };
                resolve(addressInfo);
              } else {
...
export const getNonce = function (addrArr, chainType) {
  return new Promise((resolve, reject) => {
    wand.request(
      "address_getNonce",
      { addr: addrArr, chainType: chainType },
      (err, val) => {
        if (err) {
  ...
  return new Promise((resolve, reject) => {
    wand.request("query_getGasPrice", { chainType: chainType }, (err, val) => {
      if (err) {
        ...
        let gasPrice = new BigNumber(val)
...
export const estimateGas = function (chainType, tx) {
  return new Promise((resolve, reject) => {
    wand.request(
      "transaction_estimateGas",
      { chainType: chainType, tx: tx },
      (err, val) => {
        if (err) {
...
export const checkAddrType = function (addr, addrInfo) {
  ...
  if (typeof addr === "string") {
    ...
    Object.keys(addrInfo).forEach((item) => {
      let has = Object.keys(addrInfo[item]).find(
        (val) => val.toLowerCase() === addr.toLowerCase()
      );
      if (has) {
        type = item;
        return type;
