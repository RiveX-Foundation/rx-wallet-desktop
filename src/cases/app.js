...
class App extends React.Component {
  ...
  getAddress = () => {
    wand.request(
      "address_get",
      { walletID: 1, chainType: "WAN", start: 0, end: 2 },
      function (err, val) {
        if (err) {
          console.log("error printed inside callback: ", err);
          return;
  ...
  getSingleBalance = () => {
    wand.request(
      "address_balance",
      { addr: this.state.addresses.slice(0, 1) },
      function (err, val) {
        if (err) {
          console.log("error printed inside callback: ", err);
          return;
  ...
  getNonce = () => {
    wand.request(
      "address_getNonce",
      { addr: `0x${this.state.address}`, chainType: "WAN" },
      function (err, val) {
        if (err) {
          console.log("error printed inside callback: ", err);
          return;
  ...
  createAccount = () => {
    wand.request(
      ...
      {
        walletID: 1,
        path: `${BIP44PATH.WAN}${addrOffset}`,
        meta: { name: `WanAccount_${addrOffset}` },
      },
      function (err, val) {
        if (err) {
          console.log("error printed inside callback: ", err);
  ...
  getAccount = () => {
    wand.request(
      "account_get",
      { walletID: 1, path: `${BIP44PATH.WAN}${addrOffset}` },
      function (err, val) {
        if (err) {
          console.log("error printed inside callback: ", err);
          return;
  ...
  updateAccount = () => {
    wand.request(
      "account_update",
      {
        walletID: 1,
        path: `${BIP44PATH.WAN}${addrOffset}`,
        meta: { name: `I have a new name again` },
      },
      function (err, val) {
        if (err) {
          console.log("error printed inside callback: ", err);
  ...
  render() {
    return (
      <div>
        ...
        <button onClick={this.getSingleBalance}>
          Show balance of 1st addr!!!
        </button>
