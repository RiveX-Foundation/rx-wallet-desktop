  ...
  start() {
    ...
    this.updater.on("update-available", (info) => {
      ...
      if (
        ...
        !this.updater.isManualCheckUpdates
      ) {
        return false;
      }
    ...
    this.updater.on("update-not-available", (info) => {
      this._logger.info(
        `Is manual checking for updates: ${this.updater.isManualCheckUpdates}`
      );
    ...
    if (process.platform === "darwin" || process.platform === "win32") {
      this.updater.on("download-progress", (progressObj) => {
        let logMsg =
          "Download speed: " +
          Math.ceil(progressObj.bytesPerSecond / 1024) +
          " kbps";
        logMsg =
          logMsg +
          " - Download " +
          parseFloat(progressObj.percent).toFixed(2) +
          "%";
        logMsg =
          logMsg +
          " (" +
