  ...
  constructor(mgr, type, opts) {
    ...
    this.window.on("close", (e) => {
      if (this.type === "main") {
        ...
        if (
          hasPendingCrossTx !== undefined ||
          hasPendingBTCCrossTx !== undefined
        ) {
          e.preventDefault();
          dialog.showMessageBox(
            this.window,
            {
              type: "info",
              buttons: [
                i18n.t("main.exitDialog.ok"),
                i18n.t("main.exitDialog.cancel"),
              ],
      ...
      if (this.type === "main") {
        ...
        if (global.chainManager && !this.isClosed) {
          if (this._idleChecker) {
            this._logger.info(
              "main window losing focus, clear idle time checker"
            );
            try {
          ...
          this._logger.info(
            "main window losing focus, start an away-from-main-window timer"
          );
          if (this._timer !== null) {
            this._logger.info(
              "Remain a timer should be cleared:" + this._timer
            );
    ...
    this.window.on("focus", () => {
      if (this.type === "main") {
        ...
        if (this._timer) {
          this._logger.info(
            "main window getting focus again, clear away-from-main-window timer"
          );
        ...
        if (global.chainManager) {
          ...
          if (this._idleChecker !== null) {
            this._logger.info(
              "Remain a idle timer should be cleared:" + this._idleChecker
            );
            clearInterval(this._idleChecker);
            this._idleChecker = null;
          }
          this._idleChecker = setInterval(() => {
            ...
            if (idleTime * 1000 > lockTimeThreshold) {
              this._logger.info(
                "user idle or away from key board, lock the wallet"
              );
              this._mgr.broadcast("notification", "uiAction", "lockWallet");
              clearInterval(this._idleChecker);
              this._idleChecker = null;
...
class Windows {
  ...
  create(type, options) {
    ...
    logger.info(
      `Create ${category} window: ${type}, owner: ${
        options.ownerId || "notset"
      }`
    );
  ...
  addDevToolsExtension() {
    ...
    let extPathArr = fs
      .readdirSync(extBasePath)
      .filter(
        (item) => fs.lstatSync(`${extBasePath}${item}`).isDirectory() === true
      );
