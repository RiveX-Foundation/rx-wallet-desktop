...
const BINARY_URL =
  "https://raw.githubusercontent.com/wanchain/wanwallet/wanchain3.0_beta/clientBinaries.json";
...
class Manager extends EventEmitter {
  ...
  async _checkForNewConfig(restart) {
    ...
    try {
      ...
      logger.info(
        `got BINARY_URL clients Gwan version: ${latestConfig.clients[nodeType].version}`
      );
    ...
    try {
      localConfig = JSON.parse(
        fs
          .readFileSync(path.join(setting.userDataPath, "clientBinaries.json"))
          .toString()
      );
    } catch (err) {}
    ...
    try {
      skippedVersion = fs
        .readFileSync(
          path.join(setting.userDataPath, "skippedNodeVersion.json")
        )
        .toString();
      console.log();
    } catch (err) {}
